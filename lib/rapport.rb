...
def find_email(email)
    ...
    person['jobs'] = Array.new
    contact_info['occupations'].each do |job|
        person['jobs'].push(job['job_title'] + " at " + job['company'])
    end
    person['sites'] = Array.new
    contact_info['memberships'].each do |site|
        person['sites'].push(site['site_name'] + ": " + site['profile_url'])
    end
    
    puts person['name'] + "\n\n"
    person['jobs'].each { |job| puts job + "\n" }
    puts "\n"
    person['sites'].each { |site| puts site + "\n" }
end